#!/usr/bin/env python3
"""
Evaluation Script for N-gram Language Model
Computes perplexity and compares different model configurations
"""
...
from config import (
    MODEL_FILE, MODEL_DIR, OUTPUT_DIR, PERPLEXITY_FILE, NGRAM_ORDER
)
...
def evaluate_sentence_probabilities(model: NGramModel, sentences: list, num_examples: int = 5):
    """Show example sentence probabilities."""
...
def compare_real_vs_shuffled(model: NGramModel, real_sentences: list, num_comparisons: int = 5):
    """Compare real sentences vs shuffled versions of the same sentences."""
    ...
    # Get sentences that are a reasonable length
    ...
    for i in range(num_comparisons):
        ...
        # Shuffle the same sentence (destroys word order)
